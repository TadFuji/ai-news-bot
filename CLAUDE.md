...
藤川さんの文章ルール（最初に結論を書く、網羅しない、均等な長さにしない、など）を、Gemini の文章生成 3 か所すべてに入れている。

- 中身は `skills/writing-rules.md`。**正本は x-morning-brief の `skills/writing-rules.md`** で、ここは写し（Actions がこのリポジトリしかチェックアウトしないため実体を置く）。手で編集しない
- 入れ方は `ai_client.writing_rules(scope)` が返す「## 文章ルール」の節を、1 次分析（`ai_client.py`）・2 次キュレーション（`curate_morning_brief.py`）・週刊コラム（`generate_weekly_column.py`）の各プロンプトへ差し込む。**件数・字数・形式の指定とルールが矛盾したら指定が勝つ**と前置きで明記している（「必ず 10 件」がルールの「網羅しない」に負けないようにするため）
- ファイルが読めなくても配信は止めない（警告を 1 行出してルールなしで生成する）
- あわせて変えたこと: 1 次分析の `summary_ja` の説明から「詳細」「情報を網羅して記述」を外した。週刊コラムは編集長アントのキャラクターを残したまま、冒頭の挨拶、「今週の学びを一言でまとめる」締め、「また来週！」、「5〜7 件」「800〜1200 字」の指定を外し、最初の段落で結論を書く構成にした（LINE の上限で切れないよう 4,000 字は超えないと指定）
- プロンプトは f-string の中で `writing_rules('...')` を呼んでいる。**引数は単引用符にすること**。CI は Python 3.11 で、f-string の外側と同じ二重引用符を中で使うと 3.11 では文法エラーになる（3.12 以降の手元では通ってしまう）

## 記事選定と実行時刻（2026-09-26 決定）
