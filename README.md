...
<p align="center">
  ...
  <p align="center">
    ...
    <a href="https://deepmind.google/technologies/gemini/"><img src="https://img.shields.io/badge/AI-Gemini%203.8%20Flash-4285F4.svg?logo=google&logoColor=white" alt="Gemini 3.8 Flash"></a>
  </p>
  <p align="center">
...
世界中の膨大なAIニュースから、**日本のビジネスリーダー**にとって真に価値のある情報だけを抽出し、日本語で要約・配信する**完全自動化パイプライン**です。
...
| **2次キュレーション** | 07:07 | Gemini 編集キュレーション（テーマ・コメント・重複排除） | **朝刊 Top 10** |
| **配信** | 07:07 | マルチチャネル同時配信 | Web / LINE / X |
| **週刊コラム** | 日曜 09:00 | AIが執筆するエッセイ形式のコラム | コラム + LINE配信 |

### 配信チャネル

- 🌐 **[GitHub Pages ポータル](https://tadfuji.github.io/ai-news-bot/)** — ダークモード対応アーカイブ（OGP画像・sitemap.xml・RSS配信・JSON-LD 構造化データ・静的プリレンダリング対応）
- 📱 **LINE** — Flex Carousel で Top 3 をカード配信（記事ボタン・行動提案・シェア付き）
- 🐦 **X (Twitter)** — スレッド形式 + OGP画像カード（`X_THREAD_MODE` で切替）
...
graph TD
    subgraph "データソース"
        RSS["66 RSS Feeds<br/>(ArXiv, HN, TechCrunch, NHK, 中国メディア, Workspace 等)"]
    end
...
│   ├── daily_rss_gemini.yml    #   Stage 2 (07:07 JST): 朝刊キュレーション + 配信
│   ├── weekly_column.yml       #   日曜コラム生成
│   ├── tests.yml               #   pytest 実行（push/PR）
│   └── lint.yml                #   ruff によるコード品質チェック
│
├── config.py                   # RSSソース (66件), AIキーワード, 設定
├── rss_client.py               # RSS フィードパーサー
├── article_extractor.py        # 記事本文の抽出（trafilatura）
├── dedup.py                    # 意味的重複排除（Jaccard 類似度）
│
...
├── collect_ai_news.py          # xAI Grok 経由のX調査収集（実験的・未自動化）
│
...
├── generators/                 # OGP画像・PDFレポート・動画生成
├── utils/                      # 補助スクリプト（PDF変換等）
├── tests/                      # pytest テストスイート
├── ai_news.db                  # SQLite データベース（蓄積記事）
...
| `XAI_API_KEY` | 任意 | xAI (Grok) API キー（X調査収集を使う場合） |

> **補足**: `X_THREAD_MODE` は秘密情報ではなくリポジトリ変数（Settings → Secrets and variables → Variables）です。`1` を設定するとX投稿をスレッド形式に切り替えます（未設定なら従来の単一投稿）。
...
4. 毎日 03:00 と 07:07 JST に自動実行されます

---

## 🧠 AI キュレーションの仕組み

Gemini 3.8 Flash を使った**2段階キュレーション**設計です。
...
- タイトル・要約を自然な日本語に翻訳
- カテゴリ分類（対話型AI / 画像・動画AI / 中国AI / ビジネス活用 / リスク・規制 / 日本市場 / 研究・技術 の7分類）
...
```python
RSS_FEEDS = [
    {"name": "Example", "url": "https://example.com/rss", "region": "米国"},
    # 現在 66 フィードを設定済み（米欧中日 / ニュース・ブログ・ラボ公式）
]
```
