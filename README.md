...
| ステージ | 時刻 (JST) | 処理内容 | 出力 |
|---------|-----------|---------|------|
| **収集・選定** | 06:47 | 66の RSSフィードをクロールし、Jev（`jev_selector.py`）が全件を「日本の一般読者が興味深いと思うか」で採点して上位10件を選定。Jev が使えない日はキーワードスコアリングで上位30件 | **選定済み10件** |
...
graph TD
    ...
    subgraph "Stage 1 — 03:00 JST"
        ...
        SCORE["jev_selector.py: Jev で読者の関心度を採点<br/>(退避時: config.py の AI_KEYWORDS)"]
