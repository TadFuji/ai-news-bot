...
重要度スコアに基づき、**必ず{max_articles}件** を以下のJSON配列で出力してください。
候補が{max_articles}件以上ある場合は厳選し、{max_articles}件未満の場合は候補の全件を採用してください。
**重要: 配列には必ず{max_articles}件（候補が{max_articles}件未満なら全件）を含めてください。5件や7件では不十分です。**
