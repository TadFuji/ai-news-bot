...
### 2. 朝刊の実行時刻を 06:47 JST から 07:07 JST へ変更
- 藤川さんの依頼。`daily_rss_gemini.yml` の cron を `47 21 * * *` → `7 22 * * *`（UTC）。毎時00分の混雑を避ける方針は維持
- 記事の対象範囲は「実行時点から24時間」なので、範囲も20分後ろへずれる。初回（9/27）は前回との重なり20分だが、過去3日の配信済みURL除外で重複は出ない
