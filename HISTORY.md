...
### 2. 朝刊の実行時刻を 07:07 JST へ変更
- 藤川さんの依頼。実際の起動元である VPS の crontab を `45 6 * * *` → `7 7 * * *`（JST）に変更。予備の `daily_rss_gemini.yml` の schedule は `47 21 * * *` → `37 22 * * *`（UTC、7:37 JST）。VPS より30分遅らせたのは藤川さんの判断（いったん `7 22` にした後に変更）
- 最初は workflow の schedule だけを変えて「7:07 に変えた」と報告したが、実際の起動は 2026-08-30 から VPS が担っていた（藤川さんの指摘で判明し、VPS も変更）
- 記事の対象範囲は「実行時点から24時間」なので、範囲も22分後ろへずれる。初回（9/27）は前回との重なり22分だが、過去3日の配信済みURL除外で重複は出ない
- 週刊コラム（日曜 09:47 JST）は変更なし
